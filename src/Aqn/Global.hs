...
import           Effectful.State.Local    (State)
...
type Impure m = (Unshared.State Global :> m, State Int :> m)
...
writeMeta (MetaVar r) x = do
  global <- Unshared.get
  Unshared.put $ global & (metas . at r) ?~ x
...
writeFun (FunVar r) x = do
  global <- Unshared.get
  Unshared.put $ global & (funs . at r) ?~ x
...
freshLocal n = do
  i <- fresh
  global <- Unshared.get
  Unshared.put $ global & (locals . at i) ?~ LocalInfo n
  pure $ Local i
...
freshLocal' = do
  i <- fresh
  global <- Unshared.get
  Unshared.put $ global & (locals . at i) ?~ LocalInfo (T.pack $ show i)
  pure $ Local i
