...
runStateST :: LastMember (ST t) m => s -> Eff (State s ': m) a -> Eff m (a, s)
...
evalStateST :: LastMember (ST t) m => s -> Eff (State s ': m) ~> Eff m
evalStateST ini act = fst <$> runStateST ini act
{-# INLINE evalStateST #-}

execStateST :: LastMember (ST t) m => s -> Eff (State s ': m) a -> Eff m s
execStateST ini act = snd <$> runStateST ini act
{-# INLINE execStateST #-}

-- | Interpret the state as an 'IORef'; this is the lifted version of 'runStateST'.
runStateIO :: LastMember IO m => s -> Eff (State s ': m) a -> Eff m (a, s)
...
evalStateIO :: LastMember IO m => s -> Eff (State s ': m) ~> Eff m
evalStateIO ini act = fst <$> runStateIO ini act
{-# INLINE evalStateIO #-}

execStateIO :: LastMember IO m => s -> Eff (State s ': m) a -> Eff m s
